...
/*
	  ...
	  sprintf((char*)buf,"%u",(unsigned int)data_humidity);
	  HAL_UART_Transmit(&huart2,buf,strlen((char*)buf),HAL_MAX_DELAY);
*/
	  ...
	  if(data_axe_acc.x>0x80000000)
	  {
		  ...
		  sprintf((char*)buf,"x -%u ",(unsigned int)data_axe_acc.x);
	  }else
	  {
	  sprintf((char*)buf,"x %u",(unsigned int)data_axe_acc.x);
	  }
	  ...
	  if(data_axe_acc.y>0x80000000)
	  	  {
	  		  ...
	  		  sprintf((char*)buf,"y -%u ",(unsigned int)data_axe_acc.y);
	  	  }else
	  	  {
	  	  sprintf((char*)buf,"y %u ",(unsigned int)data_axe_acc.y);
	  	  }
...
/*
	  ...
	  //IKS01A2_MOTION_SENSOR_GetAxesRaw(IKS01A2_LSM6DSL_0,MOTION_GYRO, &axe_gyro);
	 // sprintf((char*)buf,"gyro \t x : %u\r\n",(unsigned int)val);
	  if(data_axe_gyro.x>0x80000000)
	  	  {
	  		  data_axe_gyro.x=0xffffffff-data_axe_gyro.x+1;
	  		  sprintf((char*)buf,"gyro : \t  -%u  ",(unsigned int)data_axe_gyro.x);
	  	  }else
	  	  {
	  	  sprintf((char*)buf,"gyro : \t  %u",(unsigned int)data_axe_gyro.x);
	  	  }
	  ...
	  if(data_axe_gyro.y>0x80000000)
	  	  {
	  		  ...
	  		  sprintf((char*)buf," -%u ",(unsigned int)data_axe_gyro.y);
	  	  }else
	  	  {
	  		 sprintf((char*)buf," %u ",(unsigned int)data_axe_gyro.y);
	  	  }
